...
public class ListGroupsCommand extends BasicCommandListener {

    public ListGroupsCommand() {
        super("listgroups", "rankshowcase.admin");
    }

    @CommandHandler
    public void onCommand(Player player, String[] arguments) {
        if (arguments.length != 2) throw new SyntaxException("/rankshowcase listgroups <track> <rank>");
        ...
        rank.groups.forEach(group -> player.sendMessage(group));
    }
}
